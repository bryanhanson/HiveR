...
%\VignetteDepends{RColorBrewer, grid, rgl, RFOC, bipartite, sna, xtable, mvbutils, FuncMap, lattice, reshape, ggplot2}
...
This document describes some features of the \texttt{HiveR} package.\footnote{\texttt{HiveR} \Sexpr{vers}}  The current release contains a core set of functions for creating and drawing hive plots.\footnote{\url{http://github.com/bryanhanson/HiveR} or \href{http://CRAN.R-project.org/package=HiveR}{CRAN.R-project.org/package=HiveR}}  Additional features are contemplated.  There may well be bugs and features that can be improved -- your comments are always welcome.\footnote{Contact info and issues tracking can be found at the web sites above}

As with any \textit{R} package, details on functions discussed below can be found by typing \texttt{?function\_name} in the \textit{R} console after installing \texttt{HiveR}.  A complete list of functions available can be seen by typing \texttt{?HiveR} and then at the bottom of the page that opens, click on the index link.
...
\hline
\emph{method} & \emph{axis length} & \emph{center hole}  & \emph{node behavior} \\
\hline
\hline
...
\end{flushleft}
\end{center}
\caption{Comparison of methods for plotting nodes\label{Method}}
\end{table}
\end{small}
...
%%%%%% Hold this material which does the 5 axis plots
%
%
...
%@
%
%Next, we are going to copy the current version of the network (EC6) and plot the axes of the copy, because the summary above shows that the axis lengths are quite different and the shorter axes will be nearly invisible if we don't plot them up at least a bit.
%
%<< E_coli_7 >>=
%EC7 <- manipAxis(EC6, method = "plot", action = c(1, 10, 1, 10, 10))
%sumHPD(EC6)
%@
%
%Now we'll make a hive panel showing this same network displayed using different methods.  The code follows; it uses the \texttt{grid} graphics systems and associated viewport concepts to create a 2 x 2 hive panel.  The resulting hive panel is Figure~\ref{'}.
%
%<< E_coli_8, echo = TRUE, fig.cap = "\\label{E_coli_8}Hive panel showing \\emph{E. coli} regulatory network with different display options" >>=
%vplayout <- function(x, y) viewport(layout.pos.row = x, layout.pos.col = y)
%#
%grid.newpage()
...
%
%<< E_coli_9,fig.cap = "\\label{E_coli_9}Hive panel showing \\emph{E. coli} regulatory network with edges encoded by genetic distance (Red edges are the closest; each set of edges plotted separately)"  >>=
...
%<< HIV2_, rgl.mov = TRUE, fig.ext = 'png', out.width = '3 in', fig.show = 'animate', fig.num = 180, interval = 0.25, aniopts = 'controls, autoplay, loop', results = 'hide', fig.cap = "Animation of HIV-human protein-protein interactions", crop = FALSE >>=
%
%load("HIVhuH3.RData")
%
%open3d(windowRect = c(0, 0, 1000, 1000))
%
%plot3dHive(HIVhuH3, ch = 10, axLabs = c("Pol", "PR", "RT", "IN"), LA = TRUE)
%
%par3d(zoom = 1.1) # avoids clipping axis labels
%M <- par3d("userMatrix")
%inc <- pi/90 # 2 degree rotation between frames (in radians)
%
%for (i in 1:180) {
%	M2 <- rotate3d(M, i*inc, 1, 1, 0)
%	par3d(userMatrix = M2)
%	Sys.sleep(0.5)
...
\texttt{HiveR} draws hive plots very quickly when using either \texttt{plotHive} or \texttt{plot3dHive}.  Figure~\ref{perf2D} shows the performance of \texttt{plotHive} on a MacBook Pro running OSX 10.7.4 using 8 Mb RAM and an Intel  i7 chip at 2 GHz.  Figure~\ref{perf3D} shows the performance for \texttt{plot3dHive}.
...
\end{center}
\caption{Performance of plot3dHive\label{perf3D}}
\end{figure}
...
\begin{enumerate}
  ...
  \item Move to an entirely different graphics system for 2D hive plots so that tool tips and various types of edge highlight/labeling is possible by mouse action.  Package \texttt{cranvas} appears to be the ticket, but it is still in development.
\end{enumerate}
